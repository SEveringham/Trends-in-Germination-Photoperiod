...
ranef(glmlatitudefit) #shows very low values - 
```
...
slope = sqrt((fixef(glmLAIfit)[[1]]["scale(LAI):TreatmentLong"]^2 + fixef(glmLAIfit)[[1]]["scale(LAI):TreatmentShort"]^2 + (fixef(glmLAIfit)[[1]]["scale(LAI):TreatmentLong"] - fixef(glmLAIfit)[[1]]["scale(LAI):TreatmentLong"])^2 )/3)

slope

emmeans(glmLAIfit, pairwise ~ LAI * Treatment)
```
...
#model as a bayesian metric with raw data for supplement
```{r}
glmgrowthformfit <- glmmTMB(No.Germinated ~ Growth.Form+Treatment+Growth.Form:Treatment + (1+Treatment|Species.x), data = RawGerminationData, family = "binomial")
